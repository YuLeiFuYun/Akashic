...
继续对公开 Akashic 精确 revision 可独立构建。最终 V2 tree-bound campaign 在 13 个适用比较中通过 12 个，随后发现计时边界不一致。Cache Lab V3 预生成 corpus，但声明 20 rounds、实际执行 1 round，最终仍有三个支配失败。V4 真正执行 20 个 fresh-cache rounds，并把磁盘 p99 扩展为 8 轮采样。16 分片在该 workload 中只保留 619/640 热探测，32 分片此前每轮只保留 21/32；两者均拒绝。Fovea V4 固定 8 分片，并在 20 个独立 clean process block 的 scope-all 正式 campaign 中通过全部 13 个适用支配比较。这组 V4 正式结果仍绑定 `0.1.0-alpha.5` 的旧静态分片预算实现，不能因为后续发布而重绑定。动态未分配预算首次随 `0.1.0-alpha.6` 发布；当前 `0.1.0-alpha.7`（`0376b960ec8abe54f2d4a9d7d66e97f395215eaf`）保留该预算模型并增加精确 eviction 回执，Fovea `develop` 已精确 pin 到该 revision，Akashic protected `core` CI run `33524658890` 通过；这些事实证明发布与宿主采用闭环，不等价于把旧 V4 性能资格迁移到当前实现。当前工作树不再把 global cost limit 静态切给 shard。每个 shard 在公开操作边界的 assigned limit 等于真实 resident cost，未分配成本保存在内部 `CAkashicAtomics` 64-bit 原子池；普通 lookup、insert、remove 与稳态 SIEVE eviction 仍只锁目标 shard。insert 在 shard lock 内从全局池领取至多 incoming cost，完成替换/淘汰后把未使用预算在解锁前归还，因此 hash routing 不再决定有效容量。只有当单个对象无法由目标 shard 当前 resident assignment 加全部 unassigned cost 容纳时，才按固定锁序进入 all-shard slow path；该路径只释放真实 cross-shard deficit，不再把剩余容量均分给非目标 shard。不同 immediate victim cost 仍用 greedy best-fit；只有 equal-cost tie 中某 shard 的合法 successor 恰好等于第一步后的剩余 deficit 时，才允许偏离原 ring tie。该规则不是一般 look-ahead：4194304 个有界研究状态中，一般 two-step 与 unrestricted tie-lookahead 都出现新回归，而 exact-successor 规则相对 immediate greedy 为 0 regression / 46153 improvement。`AKASHIC-CT-068` 再用真实 Swift shard 状态机验证 two-victim forecast，`CT-069/070` 分别固定 resize 与 cross-shard insert 的 `[1,6]` / `[1,8]`, deficit=9 反例。全局 limit change、filtered purge、clear 与 aggregate snapshot 继续使用 configuration lock → ascending shard locks 的唯一锁序。历史 V4 正式 campaign 仍绑定 `0.1.0-alpha.5` 的旧实现。对生产 alpha.7 的新 20-block Cache Lab 重跑只有 hot-scan throughput 对 LRUCache 未跨过 1.20× 支配下界（约 1.162×，其余主比较通过），因此旧排名不能迁移。版本二分进一步把主要增量成本定位到 eviction-reporting 变更：普通 `insert` 即使不请求回执，仍携带 nil optional collector 穿过本地 SIEVE 淘汰。当前工作树把普通 insertion 与 reporting insertion 物理分离，避免在非回执热路径携带 collector；精确 victim 回执语义保持不变。该实现已经被收敛为精确 alpha.7 上的 4 文件发布最小候选 `117ae5fb9aae74a43a326ba0ab27d177ed4fe6890d3f575a7328b49e339636a3` / 411 文件，并独立通过 Akashic 全门禁和 Fovea Seatbelt 生产组合 965/965。最终冻结 Fovea 树后，以这一精确 4 文件候选作为 edited dependency 的 governed 20-block campaign 接受 20/20 clean blocks，13/13 主比较全部满足 dominance，hot-scan 对 LRUCache 中位比约 1.249×、95% 比率区间约 1.240×–1.263×。该结果仍只证明候选机制和宿主效果：edited dependency 令 `sourceResolutionBound=false`、`bestClaimEligible=false`，必须发布 immutable Akashic revision、由 Fovea 精确 pin 并在 clean source-control resolution 上重跑，才能形成新的当前实现性能资格。与该 4 文件候选分离的 CT-141 测试治理修复把无时钟预算的 `2,000 × Task.yield()` 等待替换为真实 deadline，并保证失败路径释放首个阻塞读；在 clean HEAD + 单测试文件补丁的隔离树上，`FileBlobStoreConcurrentReadTests` 九项 suite 连续 20 次实际执行时 CT-141 每次均 started/passed，随后同一树完整 `verify.sh` 通过。该测试修复不属于 alpha.7 4 文件发布补丁。
...
`DurableFileWriter` 的 package-only syscall table 可替换同步 `open(2)`、`write(2)`、`fsync(2)`、`close(2)` 和同目录 `rename(2)`；生产入口始终绑定真实 Darwin 调用，外部消费者不可访问。注入门覆盖临时文件 open 失败、部分写入、write/fsync `EINTR`、前缀写入后的 `ENOSPC`、file-fsync 失败、close 失败、rename `ENOSPC`、rename 后父目录 open 失败和 directory-fsync 失败；另有非 root macOS witness 真实把父目录降为 `0500`，要求生产临时文件 `open(O_CREAT|O_EXCL)` 以 `EACCES`/`EPERM` 在任何 mutation 前失败，旧目标逐字节不变且不留下 `.durable-tmp-*`。该 witness 只闭合 parent-mode create/open denial，不代表 ACL、different-owner、directory-open 或挂载文件系统 open 行为。package-only `renameObserver` 只在 rename 真正成功后触发：rename 自身失败时保持 false，而父目录 open/directory-fsync 的 post-rename 错误必须为 true。上层 manifest writer据此区分“尚未发布”与“磁盘 authority 已可能领先 actor”的错误边界；后一类错误冻结当前 writer instance，原始底层错误仍原样返回。
...
`Tools/Resource/capture_segmented_profile_locality_recovery.py` 把 schema5 V1 public profile 与 V2/V3/V4 research profiles 放进同一 source-identity-bound release capture，并要求首尾源码身份一致、logical authority/reopen exact、ownership/corruption controls 全部成立后才生成跨 profile 比较。512-key checkpoint workload 中 V2/V3/V4 相对 V1 的 logical metadata write ratio 都约为 1.0，说明仅更换 binary base / compound profile 并不会消除 foreground small-object metadata write amplification；相反，16,384-entry、0/1/4/16/64 run-depth 的 recovery matrix 显示 binary-base profiles 明显缩小 base/referenced bytes，并在当前 release mechanism run 中降低 segmented replay wall time。

同一 release probe 随后完成 V1/V2/V3 full compaction geometry：18 个 wide/hot/tombstone、hot/checkpoint、4/16/64-run、1,024/16,384/99,488 live-entry 与 64 KiB foreground-payload 场景共 108 个 baseline/compaction 样本，所有 fresh-reopen/authority 检查通过。相对 V1，V2 在全部 18 个场景把 compaction extra process-visible filesystem write 降到约 **0.601–0.634×**、instruction delta 降到约 **0.777–0.852×**、final base 降到约 **0.600–0.604×**；V3 分别约 **0.576–0.610×**、**0.619–0.820×** 与 **0.575–0.579×**。单轮 foreground p95 曾出现 V2 8.38× / V3 3.38× 的尖峰，因此没有后验忽略：对六个最差/边界场景再做 5 repetitions、180 samples 的定向 falsification。修正版 capture schema 4 与 full matrix 使用完全相同的 release probe SHA-256 `7d4357a242c441919ceec4b38de006a05846dd1cd94025c4655ce976bf29f6de`；排除 capture/verify harness 三项差异后 419 个产品域文件逐项相同，60/60 cross-profile physical/logical authority 对照精确成立。定向重复中 V2/V1 compaction-on p95 比值 30 个样本的中位为 **0.986**、V3 为 **0.991**，原 8.38× / 3.38× 尖峰没有复现；仍分别存在最大 1.284× / 1.505× 的小样本 wall-time 尾部，因此这只能证伪“可重复的大幅 profile latency regression”，不能证明正式 latency non-regression。capture 的 profile 顺序固定且全部报告仍声明 `formalPerformance=false`。

V4 stable-prefix/compound 的已保留证据表明其主要价值是把 64-run hard-boundary 的 validation/write work 提前摊销，而不是降低 intervention 的总 regular-write bytes；automatic admission 已拒绝 descriptor 弱收益、byte expansion 与被 suffix recurrence 侵蚀的候选，并在 hard cap 由同步 liveness backstop 接管，但 trigger 仍是静态 run-count 阈值，不是基于 host load/foreground tail 的自适应调度。基于这些结果仍不做 profile promotion。当前证据支持把 promotion 判据从“不切实际地要求 profile 编码本身改善普通小对象写量”收窄为“普通 foreground 不出现实质回归，同时 maintenance/recovery 获得可重复净收益且 authority/crash/device-I/O 门全部成立”；stable-device physical I/O、能耗/尾延迟与 V4 调度策略仍是独立门禁。
...
- 真正设备断电；
- 除已闭合的 parent-mode temporary create/open denial 外，ACL、different-owner/directory-open、owner 迁移，以及由真实文件系统触发的 `fsync`、directory `fsync`、rename 和 close 错误；
- 多进程 reader；
- 稳定真机的物理 I/O、RSS/FD 复核、能耗和长期资源包络；
- Fovea host transaction、revoke 与 final-delivery 组合性质现由 Fovea 自有的 source-bound semantic receipt 固化，Akashic 持久镜像并验证 receipt、五项义务、测试源摘要与 alpha.7 精确 revision；CT-022–026 因此进入 `implemented-local-scope-limited`，但不扩张为 protected-release 或 physical-device 资格。
