...
up before authority is restored. A separate production-path witness delegates both
file and parent-directory synchronization to the real `fsync(2)`, classifies the
second descriptor with `fstat(2)` as a directory, and proves that successful kernel
directory synchronization occurs only after the replacement rename is visible.
Different-owner transitions and real close-error behavior remain outside this witness.
