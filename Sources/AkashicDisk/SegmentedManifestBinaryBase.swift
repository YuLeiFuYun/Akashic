...
/// Package-internal fixed-width immutable base candidate for the explicit schema5 V2 profile.
///
/// This is deliberately independent from the ResourceProbe oracle so differential evidence can
/// compare two implementations of the same wire contract. A durable file is still only physical
/// candidate state until a validated segmented root references its descriptor.
package enum SegmentedManifestBinaryBaseV1 {
    package static let magic = Data("AKBSV001".utf8)
    package static let version: UInt8 = 1
    package static let headerBytes = 64
    package static let recordBytes = 96
