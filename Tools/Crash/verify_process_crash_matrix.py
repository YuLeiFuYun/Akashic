...
def main() -> int:
    ...
    for point in POINTS:
        with tempfile.TemporaryDirectory(prefix="akashic-process-crash-") as temporary:
            root = Path(temporary) / "store"
            started = time.monotonic_ns()
            crashed = subprocess.run(
                [str(binary), "crash", str(root), point],
                capture_output=True,
                text=True,
                check=False,
            )
    ...
    result = {
        "schemaVersion": 1,
        "matrixID": "AKASHIC-FILE-BLOB-PROCESS-CRASH-MATRIX-V1",
    ...
    print(f"Akashic process crash matrix: cases={len(cases)} errors={len(errors)}")
    for error in errors:
